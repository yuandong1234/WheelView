...
public class WheelView extends View {
     ...
     */
    private static final int[] SHADOWS_COLORS = new int[]{0xeeefea, 0xffffff, 0xeeefea};

    /**
     * Additional items height (is added to standard text item height)
     */
    private static int ADDITIONAL_ITEM_HEIGHT = 54;
    ...
    private void initResourcesIfNecessary() {
        ...
        if (dividerPaint == null) {
            ...
            dividerPaint.setColor(0xFFd5d5d5);
            dividerPaint.setStrokeWidth(1);
        }

        if (centerDrawable == null) {
            centerDrawable = getContext().getResources().getDrawable(R.drawable.bg_wheel_text_center);
        }
    ...
    private int getRealMaxItemWidth() {
        WheelAdapter adapter = getAdapter();
        if (adapter == null) {
            return 0;
        }
        ...
        for (int i = 0; i < count; i++) {
            String text = adapter.getItem(i);
            float textWidth = (float) Math.ceil(Layout.getDesiredWidth(text, itemsPaint));
...
//		drawCenterRect(canvas);
     ...
     * @param canvas the canvas for drawing
     */
    private void drawCutOffRule(Canvas canvas) {
        int center = getHeight() / 2;
        int offset = getItemHeight() / 2;
        canvas.drawLine(0, center - offset, getWidth(), center - offset, dividerPaint);
        canvas.drawLine(0, center + offset, getWidth(), center + offset, dividerPaint);
    }
